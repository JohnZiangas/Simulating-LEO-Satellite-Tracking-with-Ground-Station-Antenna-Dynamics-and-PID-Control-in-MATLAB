...
load('Elevation_Values(1).mat', 'elev'); % Load the desired azimuth data from the file

%% Creating the transfer function of the motor
% Define motor parameters (example values for demonstration)
K_t = 0.35;    % Torque constant (Nm/A) - proportional to the torque produced per ampere
J = 0.08;      % Moment of inertia (kg*m^2) - resistance to angular acceleration
B = 0.04;     % Damping coefficient (Nm/(rad/s)) - friction-like resistance
R_a = 2;      % Armature resistance (Ohms) - resistance of the motor windings
L_a = 0.012;   % Armature inductance (H) - inductance of the motor windings
K_b = 0.35;    % Back EMF constant (V/(rad/s)) - proportional to speed-induced voltage
...
Kp = 0.8392;  % Proportional gain
Ki = 1.8872;  % Integral gain
...
desired_elevation = elev; % Load the azimuth values

% Define the time vector for simulation
t = 1:1:length(desired_elevation); % Time in seconds (sampled every second)

% Simulate the closed-loop response to the desired azimuth angle
[actual_elevation, ~] = lsim(G_closed_loop, desired_elevation, t); % Linear simulation
...
plot(t / 60, actual_elevation, 'b-', 'LineWidth', 2); % Blue solid line for actual azimuth
hold on;

% Plot the desired azimuth (satellite movement)
plot(t / 60, desired_elevation, 'r--', 'LineWidth', 2); % Red dashed line for desired azimuth

% Add grid and legend
grid on;
legend('Actual Elevation (Antenna)', 'Desired Elevation (Satellite)', ...
       'FontSize', 11, 'Location', 'Best'); % Add legend with appropriate size

% Add title and axis labels with proper formatting
title('Elevation PID Controller', 'FontSize', 14, 'FontWeight', 'Bold');
xlabel('Time (Minutes)', 'FontSize', 12); % X-axis represents time in minutes
ylabel('Elevation Angle (Degrees)', 'FontSize', 12); % Y-axis represents azimuth angle
...
ylim([min(desired_elevation) - 5, max(desired_elevation) + 5]); % Limit Y-axis to the azimuth range
...
title("Step Response of the Elevation Control System", 'Interpreter', 'latex', 'FontSize', 16); % Title
